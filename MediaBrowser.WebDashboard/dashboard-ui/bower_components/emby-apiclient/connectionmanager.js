...
define(["events", "apiclient", "appStorage"], function (events, apiClientFactory, appStorage) {
    ...
    function ajax(request) {
        ...
        return request.headers = request.headers || {}, console.log("ConnectionManager requesting url: " + request.url), getFetchPromise(request).then(function (response) {
            return console.log("ConnectionManager response status: " + response.status + ", url: " + request.url), response.status < 404 ? "json" === request.dataType || "application/json" === request.headers.accept ? response.json() : response : Promise.reject(response)
        }, function (err) {
    ...
    var defaultTimeout = 2e4,
        ...
        ConnectionManager = function (credentialProvider, appName, appVersion, deviceName, deviceId, capabilities, devicePixelRatio) {
            ...
            self.connectUser = function () {
                return connectUser
            }, self._minServerVersion = "3.2.13", self.appVersion = function () {
                return appVersion
                ...
                if (existingServer.DateLastAccessed = (new Date).getTime(), existingServer.LastConnectionMode = ConnectionMode.Manual, existingServer.ManualAddress = apiClient.serverAddress(), apiClient.serverInfo(existingServer), apiClient.onAuthenticated = function (instance, result) {
                        onAuthenticated(instance, result, {}, !0)
                ...
                return updateDevicePromise.then(function () {
                    return apiClient.getCurrentUser().then(function (user) {
                        return params.embyUserName = user.Name, "81f53802ea0247ad80618f55d9b4ec3c" === user.Id.toLowerCase() && "21585256623b4beeb26d5d3b09dec0ac" === params.serverId.toLowerCase() ? Promise.reject() : ajax({
                            ...
                            return console.log("getRegistrationInfo response: " + status), 404 > status ? (appStorage.setItem(cacheKey, JSON.stringify({
